...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    // 1. Build header string: "blob 16\0", "tree 16\0", or "commit 16\0"
    const char *type_str = (type == OBJ_BLOB) ? "blob" :
                           (type == OBJ_TREE) ? "tree" : "commit";
    char header[64];
    int hdr_len = snprintf(header, sizeof(header), "%s %zu", type_str, len) + 1;
    // +1 to include the null byte in hdr_len
    ...
    // 3. Compute hash of the full object
    ...
    snprintf(dir, sizeof(dir), "%s/objects/%.2s", PES_DIR,
             path + strlen(OBJECTS_DIR) + 1);
    // Simpler: just build the dir from OBJECTS_DIR + first 2 hex chars
    char hex[HASH_HEX_SIZE + 1];
...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    // 2. Verify integrity: recompute hash and compare to filename
    ObjectID computed;
